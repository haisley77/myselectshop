...
public class UseTimeAop {
    ...
    @Before("product() || folder() || naver()")
    public void beforeAdvice(JoinPoint joinPoint) {
        log.info("AOP 적용됨 - " + joinPoint.getSignature().toShortString());
    }


    @Around("product() || folder() || naver()") // joinPoint.proceed()로 실제 메서드를 호출하는 시점 제어 가능
    public Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            // 핵심기능 수행
            Object output = joinPoint.proceed();    // 해당하는 요청 controller의 메서드 수행
            return output;
        } finally {
            // 측정 종료 시간
